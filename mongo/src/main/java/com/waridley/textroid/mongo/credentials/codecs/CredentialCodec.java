...
//					reader.skipValue();
    ...
    public void encode(BsonWriter writer, Credential credential, EncoderContext encoderContext) {
        if (credential instanceof OAuth2Credential) {
            ...
            encodeAbstractCredential(writer, credential, encoderContext);
        }

    }

    private void encodeAbstractCredential(BsonWriter writer, Credential credential, EncoderContext encoderContext) {
        writer.writeStartDocument();
    ...
    private void readAdditionalValues(BsonReader reader, String fieldName, DecoderContext decoderContext, Map<String, Object> additionalValues) {
        if (reader.getCurrentBsonType() == BsonType.OBJECT_ID) additionalValues.put(fieldName, reader.readObjectId());
        ...
        else if (reader.getCurrentBsonType() == BsonType.DECIMAL128)
            additionalValues.put(fieldName, reader.readDecimal128());
            ...
            additionalValues.put(fieldName, decoderContext.decodeWithChildContext(codecRegistry.get(Document.class), reader));
        else if (reader.getCurrentBsonType() == BsonType.BOOLEAN) additionalValues.put(fieldName, reader.readBoolean());
        else if (reader.getCurrentBsonType() == BsonType.DATE_TIME)
            additionalValues.put(fieldName, reader.readDateTime());
        else if (reader.getCurrentBsonType() == BsonType.BINARY)
            additionalValues.put(fieldName, reader.readBinaryData());
        else if (reader.getCurrentBsonType() == BsonType.DB_POINTER)
            additionalValues.put(fieldName, reader.readDBPointer());
        else if (reader.getCurrentBsonType() == BsonType.JAVASCRIPT)
            additionalValues.put(fieldName, reader.readJavaScript());
        else if (reader.getCurrentBsonType() == BsonType.JAVASCRIPT_WITH_SCOPE)
            additionalValues.put(fieldName, reader.readJavaScriptWithScope());
        ...
        else if (reader.getCurrentBsonType() == BsonType.REGULAR_EXPRESSION)
            additionalValues.put(fieldName, reader.readRegularExpression());
        else if (reader.getCurrentBsonType() == BsonType.TIMESTAMP)
            additionalValues.put(fieldName, reader.readTimestamp());
            ...
            if (type == BsonType.STRING) while (reader.getCurrentBsonType() == BsonType.STRING)
                additionalValues.put(fieldName, reader.readString());
            //TODO etc...
            reader.readEndArray();
        } else {
            reader.skipValue();
